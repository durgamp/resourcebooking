...
import { X, Calendar, User, Package, Layers, Activity, AlertCircle } from 'lucide-react';
...
export const BookingForm: React.FC<BookingFormProps> = ({
    ...
    const [formData, setFormData] = useState({
        reactorSerialNo: reactorSerialNo,
        team: Team.CDS,
        productName: '',
        stage: '',
        batchNumber: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (formData.status === BookingStatus.ACTUAL) {
            ...
            if (existingActual) {
                setError(`Security Violation: An 'Actual Work Log' (ID: ${existingActual.id}) already exists for this period. Only one actual log is permitted per interval.`);
                return;
            }
        }
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300">
                    ...
                    <div>
                        ...
                        <div className="flex items-center mt-1 space-x-2">
                            <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] font-bold rounded uppercase tracking-wider">
                                Resource: {formData.reactorSerialNo}
                            </span>
                            <span className="text-slate-400 text-xs">| Secure Transaction Log</span>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            <select
                                ...
                                className="w-full rounded-xl border-slate-200 bg-slate-50 py-3 text-sm focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                                required
                            >
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            placeholder="e.g. Active Pharmaceutical Ingredient X"
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-tight flex items-center">
                            <Activity size={14} className="mr-2 text-slate-400" /> Unit Operation
                        </label>
                        <input
                            type="text" required
                            value={formData.operation}
                            onChange={e => setFormData({ ...formData, operation: e.target.value })}
                            placeholder="Thermal Cracking / Distillation"
                            className="w-full rounded-xl border-slate-200 bg-slate-50 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-tight flex items-center">
                                <Calendar size={14} className="mr-2 text-slate-400" /> Start Window
                            </label>
                            <input
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-tight flex items-center">
                                <Calendar size={14} className="mr-2 text-slate-400" /> End Window
                            </label>
                            <input
                                type="datetime-local" required
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            placeholder="e.sharma@pharma-corp.com"
                            className="w-full rounded-xl border-slate-200 bg-slate-50 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                        />
                    </div>
                </form>
                ...
                <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-end items-center space-x-4">
                    ...
                    <button
                        ...
                        className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-2xl shadow-xl shadow-blue-600/30 transition-all transform active:scale-95 flex items-center"
                    >
