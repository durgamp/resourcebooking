...
import { ChevronLeft, ChevronRight, Filter, Info, Plus } from 'lucide-react';
...
export const CalendarView: React.FC<CalendarViewProps> = ({ reactors, bookings, downtimes, onAddBooking }) => {
                    ...
                    >
                      ...
                      <div className="space-y-1 relative z-10">
                        ...
                        {dayBookings.map(b => (
                          <div 
                            key={b.id} 
                            className={`${b.status === BookingStatus.ACTUAL ? 'bg-blue-500' : 'bg-amber-400'} text-white text-[10px] p-1 rounded shadow-sm font-medium truncate`}
                            title={`${b.productName} - ${b.team}`}
                          >
                            {b.productName} ({b.status === BookingStatus.ACTUAL ? 'A' : 'P'})
                          </div>
                        ))}
                      </div>
                    </td>
